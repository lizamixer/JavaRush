...
public class Z24_TypeConversion {
    {
    ...
    public static void main(String... args) throws IOException {
        try (FileOutputStream outputStream = new FileOutputStream(args[0]);
             InputStream is = Z24_TypeConversion.class.getClassLoader().getResourceAsStream(args[1])) {
            ...
            switch (s) {
                case "a": {
                    result = new Z24_TypeConversion().new A();
                    break;
                }
                case "b": {
                    result = new Z24_TypeConversion().new B();
                    break;
                }
                case "c": {
                    result = new Z24_TypeConversion().new C();
                    break;
                }
            }

            /*
            здесь была ошибка приведения типов.
            result типа Example. Если он А, то мы не можем сделать приведение к С потому что он подкласс, но мы можем сделать приведение с С,
            если он С
             */
